...
class Agent():
	def __init__(self, env):
		...
		self.demo_memory = Memory(3000, (28, ), env.action_space.shape[0])
...
if __name__ == "__main__":
	...
	for demo in range(60):
		...
		while not dones:
			...
			if action_choice == 1:
				...
				action = action + np.random.normal(scale=0.01)
			elif action_choice == 2:
				action = env.action_space.sample()

			next_state, _, dones, info = env.step(action)

			dist_eef_goal = np.linalg.norm(next_state["achieved_goal"]-next_state["desired_goal"])

			if info["is_success"] == 0.0:
				done = False
				reward = -(5.0 * dist_eef_goal)
	...
	wandb.init(project="FetchSlide-v1", entity="Vidharth")
	...
	for train in range(100000):
		...
		if train % 100 == 0:
			...
			local_success_rate_1 = np.mean(total_success_rate[:, -1])
			...
			for _ in range(10):
				...
				total_success_rate.append(per_success_rate)
			total_success_rate = np.array(total_success_rate)
			local_success_rate_2 = np.mean(total_success_rate[:, -1])

			success_rate.append(np.max([local_success_rate_1, local_success_rate_2]))
			wandb.log({"Success Rate": success_rate[-1]})

			if success_rate[-1] >= max(success_rate):
				agent.save_networks()

			if success_rate[-1] >= 0.6:
				agent.save_networks()
				exit()
